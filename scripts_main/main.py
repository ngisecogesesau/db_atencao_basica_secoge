...
from scripts_sql.transformacoes.profissionais_equipes_duckdb import create_servidores_table, create_equipes_table, create_tipo_equipe_table, update_equipes_table, update_servidores_table, create_equipes_usf_mais_table, create_gerentes_table, update_equipes_usf_mais_table, update_gerentes_table
...
def process_data(engine, schemas):
    ...
    # Atualiza tabela gerentes no duckdb
    df_update_gerentes = update_gerentes_table(con)
    ...
    try:
        ...
        df_update_gerentes.to_sql('gerentes', engine, schema='profissionais_equipes', if_exists='replace', index=False)
        logger.info("Tabela 'gerentes' atualizada no esquema 'profissionais_equipes' do banco de dados PostgreSQL com sucesso.")
